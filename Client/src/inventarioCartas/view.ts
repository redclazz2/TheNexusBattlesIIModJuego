export default class InventarioView {
  ...
  init = () =>{
    ...
    if(newElement != null) newElement.innerHTML = `<div class="container-inventory">
  ...
  llenarCartasEnHTML(cartas:any,selected_array:Array<string>) {
    try {
      ...
      cartas.forEach((carta:any) => {
        ...
        if(container!=null) container.appendChild(cardDiv);

        // Evento click al botón "Seleccionar"
        const seleccionarBtn = cardDiv.querySelector('.seleccionarBtn') as HTMLButtonElement;
  ...
  mostrarCodigo(codigo:any) {
    const codigoCarta = document.getElementById('codigoCarta');
    const codigoTexto = document.getElementById('codigoTexto');
    if(codigoTexto != null) codigoTexto.textContent = codigo;
    if(codigoCarta != null) codigoCarta.style.display = 'block';
  }
}
